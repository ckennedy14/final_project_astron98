...
for _, row in df.iterrows():
    ax2.scatter(row['Entry Speed (km/s)'], row['Peak ZHR'],
                color=colors[row['Parent Body Type']], s=160, zorder=3, edgecolors='white', linewidths=0.6)
    ax2.annotate(row['Shower'], (row['Entry Speed (km/s)'], row['Peak ZHR']),
                 textcoords='offset points', xytext=(7, 4), color='#C8D6F0', fontsize=10)
...
# Legend
legend_patches = [mpatches.Patch(color='#E8A838', label='Comet origin'),
                  mpatches.Patch(color='#5B8FE8', label='Asteroid origin')]
fig.legend(handles=legend_patches, loc='lower center', ncol=2,
           frameon=False, fontsize=11, labelcolor='#C8D6F0', bbox_to_anchor=(0.5, -0.02))

fig.suptitle('Meteor Shower Comparison', color='white', fontsize=16, fontweight='bold', y=1.01)
plt.tight_layout(rect=[0, 0.06, 1, 1])
plt.savefig('meteor_chart.png', dpi=150, bbox_inches='tight', facecolor='#0A0E2A')
plt.show()
